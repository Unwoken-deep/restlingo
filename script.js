document.addEventListener('DOMContentLoaded', () => {
    ...
    let activeCards = cards.filter(c => c.active !== false);
    let currentIndex = 0;
    ...
    function updateCounter() {
        if (counter) counter.textContent = activeCards.length;
    }
    ...
    function showCard() {
        ...
        if (activeCards.length === 0) {
            ...
            translationElement.textContent = 'Добавь карточку';
            updateCounter();
            return;
        }
    ...
    function nextCard() {
        activeCards = cards.filter(c => c.active !== false);
    ...
    if (nextArrow) nextArrow.addEventListener('click', nextCard);
    if (prevArrow) prevArrow.addEventListener('click', prevCard);
    ...
    if (addButton) {
        addButton.addEventListener('click', () => {
            ...
            if (w && t) {
                cards.push({ word: w, translation: t, active: true });
                saveCards();
                activeCards = cards.filter(c => c.active !== false);
                newWordInput.value = '';
    ...
    if (deleteButton) {
        deleteButton.addEventListener('click', () => {
            ...
            const ok = confirm('Удалить эту карточку?');
            if (!ok) return;

            const realIndex = cards.indexOf(activeCards[currentIndex]);
            if (realIndex !== -1) {
